...
def build_table_chunks(df: pd.DataFrame) -> list[dict]:
    ...
    for (schema, table), group in grouped:
        ...
        # ── FK hints (one line per FK column) ────────────────────
        fk_lines = []
        if "is_foreign_key" in group.columns:
            fk_rows  = group[group["is_foreign_key"] == 1]
            for _, row in fk_rows.iterrows():
                ref = f"{row['referenced_schema']}.{row['referenced_table']}.{row['referenced_column']}"
                fk_lines.append(f"  - {row['column_name']} -> {ref}")

        # ── assemble chunk content ────────────────────────────────
        content_lines = [f"Table Name: {full_name}"]
        ...
        if pd.notna(keywords) and str(keywords).strip() and str(keywords).strip() != "nan":
            content_lines.append(f"Keywords: {str(keywords).strip()}")
            
        content_lines.append("Columns:")
        content_lines.extend(col_lines)

        if fk_lines:
            content_lines.append("Foreign Keys:")
            content_lines.extend(fk_lines)
